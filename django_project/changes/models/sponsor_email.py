...
from changes.models import SponsorshipLevel
...
class SponsorEmail(models.Model):
    ...
    def get_recipients(self):
        ...
        from changes.models import SponsorshipPeriod  # Import here to avoid circular imports
    ...
    def get_recipient_emails(self):
        ...
        if self.email_type in ['admin', 'both']:
            admin_emails = sponsors.exclude(sponsor_email__exact='').values_list('sponsor_email', flat=True)
            emails.extend(admin_emails)
        
        if self.email_type in ['tech', 'both']:
            tech_emails = sponsors.exclude(tech_email__exact='').values_list('tech_email', flat=True)
